<?php
	...
	if( $g_project_id == ALL_PROJECTS )
  {
	...
	$f_skip_first = gpc_get_bool( 'skip_first' );
	...
	function get_enum_column_value( $p_name, $p_row, $p_default )
	{
		...
		# First chance, search element in configuration enumeration string
	...
	function get_date_column_value( $p_name, $p_row, $p_default )
	{
		...
		return is_blank( $t_username ) ? $p_default : strtotime( $t_date );
	}
	...
	function get_user_column_value( $p_name, $p_row, $p_default )
	{
		...
		return is_blank( $t_username ) ? $p_default : user_get_id_by_name( $t_username );
	}
	...
	{
		global $f_columns;
		$t_column = array_search( $p_name, $f_columns );
		return ( $t_column === false || !isset( $p_row[$t_column] ) ) ? $p_default : trim( $p_row[$t_column] );
	}
	...
	function column_value_exists( $p_name, $p_row )
	{
		global $f_columns;
		$t_column = array_search( $p_name, $f_columns );
		return $t_column !== false && isset( $p_row[$t_column] );
	}
	...
	$t_failure_count = 0;
	$t_error_messages = '';
	$t_bug_exists = array_search( 'id', $f_columns );
	foreach( $t_file_content as $t_file_row )
	{
		...
		$t_file_row = read_csv_row( $t_file_row, $t_separator );
		...
		$t_bug_data->category =
		 get_column_value( 'category', $t_file_row, $t_default->category );
		$t_bug_data->priority =
		...
		if( $t_bug_id )
		{
			...
			foreach( custom_field_get_linked_ids( $g_project_id ) as $t_id )
			{
				...
				if( !column_value_exists( $t_def['name'], $t_file_row ) ) continue;

				# Prepare value
				$t_value = get_column_value( $t_def['name'], $t_file_row );
